...
public class MemoryCache {
	...
	private long limit = 99999999;// max memory in bytes
	...
	public Bitmap get(String imgId) {
		try {
			if (!cache.containsKey(imgId)){
				return null;
			}
	...
	public void put(String imgId, Bitmap bitmap) {
		try {
			if (cache.containsKey(imgId)){
				size -= getSizeInBytes(cache.get(imgId));
			}
			cache.put(imgId, bitmap);
